...
#define SETTINGS_SERVICE_UUID "91bad492-b950-4226-aa2b-4ede9fa42f59"
...
#define RENAME_CHAR_UUID     "0d563a58-1961-48ce-8ce9-dfc43db5c4ba"
...
void sendFallAlert() {
  if (device_connected) {
    fallCount++;
    String payload = "{\"type\":\"FALL\",\"device_id\":\"" + deviceName + "\","
...
void sendEmergencyAlert() {
  if (device_connected) {
    String payload = "{\"type\":\"EMERGENCY\",\"device_id\":\"" + deviceName + "\","
                     "\"fall_count\":" + String(fallCount) +
                     ",\"timestamp\":\"" + getReadableTime() + "\"}";
    pFallAlert->setValue(payload.c_str());
    pFallAlert->notify();
...
void sendBatteryLevel() {
  if (device_connected) {
    String payload = "{\"type\":\"BATTERY\",\"device_id\":\"" + deviceName + "\","
...
class NameChangeCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
        ...
        if (value.length() > 0) {
            ...
            preferences.begin("rescu-settings", false); 
            preferences.putString("devName", value.c_str());
            preferences.end();
            
            Serial.println("[BLE] Name updated. Restarting...");
            delay(500); 
            ESP.restart();
        }
    }
...
void setup() {
  ...
  // ── BLE init ──────────────────────────────────────────────────────────
  preferences.begin("rescu-settings", true);
  deviceName = preferences.getString("devName", "RESCU_DEVICE"); 
  preferences.end();
  ...
  BLEService *pSettings = pServer->createService(SETTINGS_SERVICE_UUID);
  BLECharacteristic *pRenameChar = pSettings->createCharacteristic(
                                        RENAME_CHAR_UUID,
                                        BLECharacteristic::PROPERTY_WRITE
                                     );
  pRenameChar->setCallbacks(new NameChangeCallbacks());
  pSettings->start();
