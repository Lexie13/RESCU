...
def trigger_emergency_email_loop(user_id, location_data="Location Unavailable"):
    try:
        ...
        for contact in parsed_contacts:
            ...
            try:
                sns_client.publish(
                    ...
                    Message=message,
                    MessageAttributes={
                        'target_email': {'DataType': 'String', 'StringValue': contact_email}
                    }
                )
                ...
                for _ in range(iterations):
                    ...
                    alert_record = table_alerts.get_item(Key={"alert_id": alert_id}).get("Item")
        ...
        if is_acknowledged:
            return {"success": True, "message": "Alert acknowledged", "notified": notified_contacts}
        else:
             return {"success": False, "error": "Loop finished, but no contact acknowledged the alert."}
